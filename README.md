...
Flare derives its painted layers directly from the matching Light Flare layers. The Peach Cream light/dark pair supplies a per-layer lightness ratio and hue movement; those changes are applied to the selected palette's existing layer color. Frame and inner glow derivation remain independent, so the layer transform cannot introduce an unrelated utility-token hue into the Flare body.
